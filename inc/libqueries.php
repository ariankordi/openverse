...
global $settings;
global $link;
$result = prepared('SELECT *, COUNT(yeah_id) AS yeah_count FROM posts LEFT JOIN users ON user_pid = post_by LEFT JOIN post_yeahs ON yeah_post = post_id LEFT JOIN communities ON community_id = post_community LEFT JOIN titles ON title_id = community_title WHERE LENGTH(post_screenshot) AND post_community IN (?) GROUP BY post_by DESC ORDER BY post_date DESC LIMIT 0,10', [join(', ', $settings['featured_communities'])]);
