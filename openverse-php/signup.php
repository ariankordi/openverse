		...
		if(isset($open_recaptcha_secret)) {
            ...
            if(!isset($open_recaptcha_secret) || $json['success'] == 0) {
                ...
                if(isset($_POST['user_nnid']) && strlen($_POST['user_nnid']) > 0) {
                    $dh = curl_init();
                    curl_setopt($dh, CURLOPT_URL, 'https://miiverse.nintendo.net/users/' . urlencode($_POST['user_nnid']) . '/favorites');
                    curl_setopt($dh, CURLOPT_RETURNTRANSFER, true);
                    $response = curl_exec($dh);
                    $dom = new DOMDocument();
                    $res = $dom->loadHTML($response);
                    $xpath = new DomXPath($dom);
                    $imgs = $xpath->query("//*[contains(concat(' ', normalize-space(@class), ' '), ' icon ')]");
                    if ($imgs->length != 0) {
                    $url = $imgs[0]->getAttribute("src");
                    if (strpos($url, "anonymous-mii.png") !== false) {
                        $mii_hash = '';
                    } else {
                        $mii_hash = str_replace("_normal_face.png", "", str_replace("https://mii-secure.cdn.nintendo.net/", "", $url));
                    }
                    $ids = $xpath->query("//*[contains(concat(' ', normalize-space(@class), ' '), ' id-name ')]");
                    $user_nnid = $ids[0]->textContent;
                    $names = $xpath->query("//*[contains(concat(' ', normalize-space(@class), ' '), ' nick-name ')]");
                    $mii_name = $names[0]->textContent;
                } else {
                    $mii_hash = '';
                    $user_nnid = '';
                    $mii_name = '';
                }
                if(!isset($_POST['screen_name']) || strlen($_POST['screen_name']) == 0) {
                    if(strlen($mii_name) > 0) {
                        $screen_name = $mii_name;
                    } else {
                ...
                $confirmation_code = str_pad(rand(0, pow(10, 4)-1), 4, '0', STR_PAD_LEFT);
                ...
                if(mysqli_error($link)) {
                    ...
                    $headers[] = 'From: Openverse Dev <no-reply@openverse-dev.pf2m.com>';
                    mail($_POST['email'], 'Confirm your Openverse account', '<html><head><title>Confirm your Openverse account</title><style>body{text-align:center}</style></head><body><img src="https://openverse.pf2m.com/assets/img/menu-logo.png" alt="Openverse"><p><i>Your Openverse account is almost complete.</i> We just need to confirm your email to finish your registration.</p><p>To confirm your email address, go to the bottom of your account settings page and enter this confirmation code: <b>' . $confirmation_code . '</b></p><p>If you need help, try <a href="https://openverse-dev.pf2m.com/account/resend">resending the confirmation email</a> or <a href="https://www.pf2m.com/contact/">contacting us here</a>.</p><br><p>Thank you for using our service! :)</p></body></html>', implode("\r\n", $headers));
                    echo '<div class="main-column center"><div class="post-list-outline"><br><img src="/assets/img/menu-logo.png"><br><p style="font-weight: bold; font-size: 20px;">Sign-up Successful!</p><p>An email has been sent to the email address you provided with instructions on how to confirm your email address.<br>You can now <a href="/account/login">log in</a> to access all of Openverse!</p><br></div></div>';
                }
                } else {
                    $error = 'The Nintendo Network ID you entered does not exist or is not set up for Miiverse.';
                    include 'signup-form.php';
                }
                curl_close($dh);
            }
