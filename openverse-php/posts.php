...
echo '<p class="reply-content-text">' . htmlspecialchars($row['reply_content']) . '</p>';
}
...
if($_SESSION['user_rank'] > 2) {
    echo '<option value="remove" data-action="/posts/' . htmlspecialchars($_GET['id']) . '/remove" data-url-id="' . htmlspecialchars($_GET['id']) . '" data-track-action="removePost">Remove</option>';
}
        echo '</select>
        <div class="form-buttons">
          <input type="button" class="olv-modal-close-button gray-button" value="Cancel">
