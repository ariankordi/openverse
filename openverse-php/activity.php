...
echo '<div class="main-column"><div class="headline"><h2><span class="activity-headline symbol">Activity Feed</span></h2><form class="search" action="/users"><input type="text" name="query" placeholder="Search Users" minlength="1" maxlength="32"><input type="submit" value="q"></form></div><div class="list post-list js-post-list" data-next-page-url="/activity?offset=';
echo $offset+50;
echo '">';
$postArray = [];
$sql = 'SELECT follow_to FROM follows WHERE follow_by = ' . mysqli_real_escape_string($link, $_SESSION['user_pid']);
$result = mysqli_query($link, $sql);
if(!mysqli_error($link)){
while ($row = mysqli_fetch_assoc($result)) {
$fresult = mysqli_query($link, 'SELECT * FROM posts LEFT JOIN users ON users.user_pid = posts.post_by LEFT JOIN communities ON communities.community_id = posts.post_community WHERE post_by = ' . mysqli_real_escape_string($link, $row['follow_to']) . ' AND post_status < 2');
while ($frow = mysqli_fetch_assoc($fresult)) {
array_push($postArray, $frow);
}}
$meSQL = 'SELECT * FROM posts LEFT JOIN users ON users.user_pid = posts.post_by LEFT JOIN communities ON communities.community_id = posts.post_community WHERE post_by = ' . mysqli_real_escape_string($link, $_SESSION['user_pid']) . ' AND post_status < 2';
$meResult = mysqli_query($link, $meSQL);
if(mysqli_num_rows($meResult)!=0){
while ($meRow = mysqli_fetch_assoc($meResult)) {
array_push($postArray, $meRow);
}}
