...
echo '</div><div class="main-column"><div class="post-list-outline"><div class="tab-container"><div class="tab2"><a class="selected" href="/titles/' . htmlspecialchars($row['community_title']) . '/' . htmlspecialchars($row['community_id']) . '">All Posts</a><a href="/titles/' . htmlspecialchars($row['community_title']) . '/' . htmlspecialchars($row['community_id']) . '/hot">Popular Posts</a></div></div>';
...
if(mysqli_num_rows(mysqli_query($link, 'SELECT post_id FROM posts WHERE post_community = ' . mysqli_real_escape_string($link, $row['community_id']) . ' AND post_status = 0 LIMIT ' . mysqli_real_escape_string($link, $offset+50) . ',1'))>0){
echo '="/titles/' . $row['community_title'] . '/' . $row['community_id'] . '?offset=' . htmlspecialchars($offset+50) . '"';
}
echo '>';
        $result = mysqli_query($link, 'SELECT * FROM posts LEFT JOIN users ON posts.post_by = users.user_pid WHERE post_community = ' . mysqli_real_escape_string($link, $row['community_id']) . ' AND post_status = 0 ORDER BY post_id DESC LIMIT ' . $offset . ',50');
